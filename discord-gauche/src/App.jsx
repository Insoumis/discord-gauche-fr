...
function App() {
  ...
  const [selectedBackground, setselectedBackground] = useState(posts.featured[0].banner)
  ...
  return (
    ...
    <div className="h-screen flex flex-col bg-noir text-gray-800 overflow-hidden font-[Grotesk]">
      <div className="absolute w-full h-[700px] bg-cover mask-b-from-10% mask-b-to-80% opacity-30 transition-all duration-500" style={{ backgroundImage: `url(${selectedBackground})` }} />
      ...
      <main className="flex flex-col py-18 overflow-hidden overflow-y-scroll text-blanc absolute top-0 bottom-0 right-0 left-0">
        ...
        <div className="flex flex-row overflow-hidden overflow-x-auto min-h-[400px]">
          {posts.featured.map((post) => (
            <div key={post.id} className="ml-6 first:w-[644px] w-[244px] overflow-hidden mb-8 shrink-0 transition-all">
              <div className="md:flex flex-col h-[400px]">
                <a onFocus={() => setselectedBackground(post.banner)} onMouseOver={() => setselectedBackground(post.banner)} href="#" className="imageAnchor outline-none p-[2px] border-2 border-transparent focus:border-blanc transition-all duration-500 text-transparent focus:border-blanc focus:transform-[matrix(0.8,0.05,0,0.8,1,0)] focus:text-blanc hover:border-blanc hover:transform-[matrix(0.8,0.05,0,0.8,1,0)] hover:text-blanc">
                  <img
        ...
        <div className="flex flex-row overflow-hidden overflow-x-auto min-h-[400px]">
          {posts.games.map((post) => (
            <div key={post.id} className="ml-6 w-[244px] overflow-hidden mb-8 shrink-0 transition-all">
              <div className="md:flex flex-col h-[400px]">
                <a onFocus={() => setselectedBackground(post.banner)} onMouseOver={() => setselectedBackground(post.banner)} href="#" className="imageAnchor outline-none p-[2px] border-2 border-transparent focus:border-blanc transition-all duration-500 text-transparent focus:border-blanc focus:transform-[matrix(0.8,0.05,0,0.8,1,0)] focus:text-blanc hover:border-blanc hover:transform-[matrix(0.8,0.05,0,0.8,1,0)] hover:text-blanc">
                  <img
                    className="w-full h-[360px] object-cover"
